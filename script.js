...
const UNLOCK_TIME = new Date("2026-09-10T18:30:00Z").getTime();
...
function startMusic() {
    ...
    if (promise) {
        promise.catch(() => {
            // Browser may still block audio until another click.
        });
    }
}
...
/* If the browser blocks audio on the first play,
   the next user click retries it. */
...
nextPhoto.addEventListener("click", () => {
    ...
    /* The click on "Next Photo" is a real user gesture,
       so browsers normally allow video.play() here. */
    video.play().catch(() => {});
});
...
/* =========================================================
   ...
   Robust version:
   - every answer gets a funny response
   - response element is created automatically if missing
   - Next works after every question
   - original question/answer HTML is preserved
========================================================= */
...
funnyQuestions.forEach((question, index) => {
    ...
    /* Hide all later questions initially. */
    if (index > 0) {
    ...
    next.addEventListener("click", () => {
        ...
        if (nextQuestion) {
            ...
            /* Q4 is the final funny question. */
            showTab("tab-4");
        }
    });
});
...
/*
   Your public Supabase URL and public anon/publishable key
   can be placed below.

   NEVER put a service-role/secret key in browser JavaScript.

   The website still works without these values; answers will
   simply be logged in the browser console until connected.
*/

const SUPABASE_URL = "https://ofzwsyqwsxpuivfsyeum.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_PkGTYWtDC55fR5IV44nNwg_WkJ6YKxU";
